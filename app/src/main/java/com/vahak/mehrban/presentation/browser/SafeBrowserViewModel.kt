...
class SafeBrowserViewModel @Inject constructor(
    ...
    fun isUrlAllowed(url: String?): Boolean {
        ...
        // 1. ALWAYS block keywords (Global override)
        if (state.value.blockedKeywords.any { normalized.contains(it.keyword.lowercase()) }) return false

        // 2. ALWAYS block blacklisted sites (Using sub-path prefix criteria)
        // If block rules hold "A.com/adult", this locks out "A.com/adult/page" cleanly
        ...
        if (state.value.filterMode == FilterMode.WHITELIST_ONLY) {
            ...
            // 🚀 THE CRITICAL PATH MATCH FIX: Using .startsWith() lets "A.com/kid" match and allow
            // "A.com/kid/video" or "A.com/kid/index.html", while blocking "A.com/adult"
