...
interface AuthRepository {
	...
	suspend fun logout()
	suspend fun syncAppInstalledStatus(): Result<Boolean>
}
...
class AuthRepositoryImpl @Inject constructor(
	...
	override suspend fun syncAppInstalledStatus(): Result<Boolean> {
		return try {
			val response = authApi.checkAppInstalledStatus()
			if (response.isSuccessful && response.body() != null) {
				val hasInstalledExercise = response.body()!!.hasInstalledExercise
				sessionManager.setHasInstalledExercise(hasInstalledExercise)
				Result.success(hasInstalledExercise)
			} else {
				Result.failure(Exception("Failed to fetch app status"))
			}
		} catch (e: Exception) {
			Timber.w(e, "Error syncing app status from server")
			Result.failure(e)
		}
	}
}