...
fun DashboardScreen(
	...
	onExerciseRewardSettingsClick: (String) -> Unit = onTimeLockClick,
	...
	DashboardScreenContent(
		...
		onExerciseRewardSettingsClick = onExerciseRewardSettingsClick,
...
fun DashboardScreenContent(
	...
	onReportClick: (String) -> Unit,
	onTimeLockClick: (String) -> Unit,
	onExerciseRewardSettingsClick: (String) -> Unit,
	onSecurityFabClick: (String) -> Unit,
	onProfileClick: () -> Unit,
) {
	...
	DisposableEffect(lifecycleOwner, settings, state.hasInstalledExercise) {
		val observer = LifecycleEventObserver { _, event ->
			if (event == Lifecycle.Event.ON_RESUME) {
				evaluatePermissions()
				if (!state.hasInstalledExercise) {
					onEvent(DashboardEvent.RefreshAppInstalledStatus)
				}
			}
		}
	...
	Scaffold(
		...
		Column(
			...
			if (state.children.isEmpty() || state.activeChild == null) {
				...
				ActionGridV2(
					hasInstalledExercise = state.hasInstalledExercise,
					showExerciseBadge = state.showExerciseBadge,
					onSettingsClick = { onSettingsClick(state.activeChild.id) },
					onReportClick = { onReportClick(state.activeChild.id) },
					onTimeLockClick = { onTimeLockClick(state.activeChild.id) },
					onExerciseClick = {
						if (state.hasInstalledExercise) {
							onExerciseRewardSettingsClick(state.activeChild.id)
						} else {
							onEvent(DashboardEvent.ExerciseClicked)
						}
					}
				)
			}
	...
	if (state.showExerciseInstallDialog) {
		val comingSoonMessage = stringResource(R.string.coming_soon)
		val exerciseInstallTitle = stringResource(R.string.exercise_install_dialog_title)
		val exerciseInstallDesc = stringResource(R.string.exercise_install_dialog_desc)
		val exerciseInstallConfirm = stringResource(R.string.exercise_install_dialog_confirm)
		val exerciseInstallDismiss = stringResource(R.string.exercise_install_dialog_dismiss)
		AlertDialog(
			onDismissRequest = { onEvent(DashboardEvent.DismissExerciseDialog) },
			containerColor = colors.surface,
			title = {
			...
			confirmButton = {
				TextButton(
					onClick = {
						onEvent(DashboardEvent.DismissExerciseDialog)
						val intent = Intent(
							Intent.ACTION_VIEW,
							"https://mehr-ban.com/mehrban-latest.apk".toUri()
						)
						context.startActivity(intent)
					}
				) {
					Text(
						exerciseInstallConfirm,
			...
			dismissButton = {
				TextButton(onClick = { onEvent(DashboardEvent.DismissExerciseDialog) }) {
					Text(exerciseInstallDismiss, color = colors.textHint)
				}
			}
		)
	}
