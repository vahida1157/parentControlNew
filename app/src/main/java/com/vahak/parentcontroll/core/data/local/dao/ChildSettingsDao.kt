...
interface ChildSettingsDao {
    ...
    // --- Time Limit Updates (Now flags is_synced = 0) ---
    @Query("UPDATE global_settings SET is_time_limit_active = :isActive, is_synced = 0, updated_at = :time WHERE child_id = :childId")
    suspend fun updateTimeLimitToggle(childId: String, isActive: Boolean, time: Long = System.currentTimeMillis())

    @Query("UPDATE global_settings SET daily_time_limit_mins = :minutes, is_synced = 0, updated_at = :time WHERE child_id = :childId")
    ...
    suspend fun updateSleepTimeToggle(childId: String, isActive: Boolean, time: Long = System.currentTimeMillis())
    ...
    suspend fun updateSleepTimeSchedule(childId: String, startTime: LocalTime, endTime: LocalTime, time: Long = System.currentTimeMillis())
